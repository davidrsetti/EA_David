...
# ══════════════════════════════════════════════════════════════════════════════
# D-2: Conversational ADR Generation
# ══════════════════════════════════════════════════════════════════════════════

import datetime
import hashlib

_ADR_DOC_BASE = "https://nexus.platform/adr#"
...
def generate_adr(
    ...
    """
    Generate a MADR-format Architecture Decision Record.

    Returns ``(madr_markdown, adr_uri)``.  The URI is deterministic from
    (domain, capability, decision) so the same decision can be updated rather
    than duplicated.
    """
    date_str = datetime.date.today().isoformat()
    slug     = hashlib.md5(f"{domain}{capability}{decision}".encode()).hexdigest()[:8]
    adr_uri  = f"{_ADR_DOC_BASE}ADR-{slug}"
    ...
    for i, opt in enumerate(options, 1):
        ...
        fit         = opt.get("fit", "")
        complexity  = opt.get("estimated_complexity", "")
        platforms   = ", ".join(opt.get("recommended_platforms", []))
        opt_risks   = "; ".join(opt.get("risks", []))
        option_rows += (
            f"| {name} | {fit} | {complexity} | {platforms} | {opt_risks} |\n"
        )

    options_table = (
        "| Option | Fit | Complexity | Platforms | Risks |\n"
        "|--------|-----|------------|-----------|-------|\n"
        + (option_rows or "| — | — | — | — | — |\n")
    )

    risks_block   = "\n".join(f"* {r}" for r in risks) if risks else "* No major risks identified"
    roadmap_block = "\n".join(f"{i}. {s}" for i, s in enumerate(roadmap, 1)) if roadmap else "1. TBD"
...
{business_goal or "A solution architecture decision is required for the capability described above."}

The decision was informed by live data from the NEXUS Enterprise Knowledge Graph including existing
applications, capability gaps, data classifications, and architecture patterns active in the
{domain} domain.
...
{rationale or "Selected based on best fit against functional requirements, NFRs, and graph-derived signals."}
...
*Generated by NEXUS SA Advisor on {date_str}. URI: `{adr_uri}`*
"""
    return madr.strip(), adr_uri


def store_adr_in_graph(
    adr_uri: str,
    domain: str,
    capability: str,
    madr_text: str,
    ...
    """
    Persist the ADR as a ``doc:ArchitectureDecisionRecord`` node in StarDog.

    Linked to the capability URI (if resolvable) via ``doc:coversCapability``.
    Falls back silently if StarDog is unavailable or the capability can't be resolved.
    """
    try:
        db = _db()
        date_str = datetime.date.today().isoformat()
        ...
        safe_text   = madr_text.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n")
        safe_domain = _safe(domain)
        safe_cap    = _safe(capability)
        safe_author = _safe(author)

        # Try to resolve capability IRI for linking
        cap_triple = ""
        if capability:
            q = f"""
            SELECT ?cap WHERE {{
                ?cap a ?t .
                ?cap <{RDFS}label> ?l .
                FILTER(LCASE(STR(?l)) = LCASE("{safe_cap}"))
            }} LIMIT 1
            """
            try:
                _, rows = db.to_rows(db.query(q))
                if rows:
                    cap_iri = rows[0].get("cap", "")
                    if cap_iri:
                        cap_triple = f'<{adr_uri}> <https://nexus.platform/doc#coversCapability> <{cap_iri}> .'
            except Exception:
                pass

        update = f"""
        PREFIX doc:  <https://nexus.platform/doc#>
        PREFIX rdfs: <{RDFS}>
        PREFIX xsd:  <http://www.w3.org/2001/XMLSchema#>

        INSERT DATA {{
            <{adr_uri}> a doc:ArchitectureDecisionRecord ;
                rdfs:label "{_safe(adr_uri.split('#')[-1])}" ;
                doc:domain "{safe_domain}" ;
                doc:capability "{safe_cap}" ;
                doc:author "{safe_author}" ;
                doc:date "{date_str}"^^xsd:date ;
                doc:madrText "{safe_text}" .
            {cap_triple}
        }}
        """
        db.update(update)
        logger.info("ADR stored in graph: %s", adr_uri)
    except Exception as exc:
        logger.warning("store_adr_in_graph: %s — ADR text returned but not persisted", exc)


def list_adrs_from_graph(capability: str = "", domain: str = "") -> list[dict]:
    """
    Query StarDog for all ADRs, optionally filtered by capability / domain.
    Returns a list of dicts with uri, label, domain, capability, author, date.
    """
    try:
        db = _db()
        cap_filter = (
            f'FILTER(CONTAINS(LCASE(?cap), LCASE("{_safe(capability)}")))'
            if capability else ""
        )
        dom_filter = (
            f'FILTER(CONTAINS(LCASE(?dom), LCASE("{_safe(domain)}")))'
            if domain else ""
        )
        q = f"""
        PREFIX doc:  <https://nexus.platform/doc#>
        ...
        SELECT ?adr ?label ?dom ?cap ?author ?date WHERE {{
            ?adr a doc:ArchitectureDecisionRecord .
            OPTIONAL {{ ?adr rdfs:label    ?label  }}
            OPTIONAL {{ ?adr doc:domain    ?dom    }}
            OPTIONAL {{ ?adr doc:capability ?cap   }}
            OPTIONAL {{ ?adr doc:author    ?author }}
            OPTIONAL {{ ?adr doc:date      ?date   }}
            {dom_filter}
            {cap_filter}
        ...
        logger.warning("list_adrs_from_graph: %s", exc)
        return []
