"""Lucide SVG icons + Material shortcodes.

Two interfaces:
  • icon(name, size=18, color="currentColor")  -> inline <svg> string
       Use inside any markdown rendered with unsafe_allow_html=True.
  • mat(name)  -> ":material/<name>:" shortcode
       Use in st.tabs(), st.button(label=...), st.metric() etc — Streamlit
       (>=1.36) renders these inline as Material Symbols.

The Lucide SVGs use stroke="currentColor" so they inherit the surrounding
text colour and can be tinted with CSS.
"""
from __future__ import annotations

# ── Lucide SVG paths (stroke-based, monoline, 24x24 viewBox) ─────────────
# Source: lucide.dev (ISC licence). Only the inner <path>/<line>/<circle>
# elements are stored; the wrapping <svg> is built in icon().
_LUCIDE: dict[str, str] = {
    ...
    "network":          '<rect x="9" y="2" width="6" height="6"/><rect x="16" y="16" width="6" height="6"/><rect x="2" y="16" width="6" height="6"/><path d="M5 16v-3h14v3"/><path d="M12 12V8"/>',
...
def icon(name: str, size: int = 18, color: str = "currentColor", stroke: float = 1.75) -> str:
    """Return an inline Lucide SVG string. Use in markdown with unsafe_allow_html=True."""
    body = _LUCIDE.get(name)
    if not body:
        return ""
    return (
        f'<span class="nx-icon">'
...
# ── Material Symbols shortcodes (used by st.tabs, buttons, metrics) ──────
# Streamlit 1.36+ renders ":material/<name>:" as a Material icon.
